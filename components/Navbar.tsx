...
import { SparklesIcon, MenuIcon, CloseIcon, ArrowRightIcon, LogOutIcon } from "./icons";
        ...
        <div className="hidden lg:flex items-center gap-4">
          {!isLoading && user ? (
            <div className="flex items-center gap-3">
              <Link
                href="/dashboard"
                className="relative inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full font-semibold text-xs text-white overflow-hidden group shadow-md shadow-violet-600/25 hover:shadow-violet-600/40 transition-all duration-300 bg-gradient-to-r from-violet-600 to-indigo-600"
              >
                <span>Dashboard</span>
                <ArrowRightIcon className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
              </Link>
              <div className="flex items-center gap-2 pl-2 border-l border-white/10">
                <span className="text-xs text-slate-300 font-medium max-w-[120px] truncate" title={user.email}>
                  {displayName}
                </span>
                <button
                  onClick={signOut}
            ...
            {!isLoading && user ? (
              <>
                ...
                <button
                  ...
                  className="w-full text-center py-2.5 text-sm font-medium text-red-400 hover:text-red-300 rounded-lg bg-white/[0.03]"
                >
